...
export default async function LearnerQuizPage({
  ...
  const { data: assessment } = await supabase
    ...
    .maybeSingle();
  if (!assessment) redirect(`/courses/${courseId}/modules/${moduleId}`);
    ...
    return (
      <main className="mx-auto flex w-full max-w-2xl flex-col gap-6 px-4 py-10">
        ...
        <p className="text-sm text-neutral-600">
          You&apos;ve used all {typedAssessment.max_attempts} attempts for this quiz without passing.
          Contact your admin to reset your attempts.
        </p>
      </main>
    );
  }
  ...
  if (pool.length === 0) {
    return (
      <main className="mx-auto flex w-full max-w-2xl flex-col gap-6 px-4 py-10">
        ...
        <p className="text-sm text-neutral-500">This quiz has no questions yet — check back later.</p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-2xl flex-col gap-8 px-4 py-10">
      ...
      <div>
        <h1 className="text-xl font-semibold">{typedModule.title} quiz</h1>
      ...
      <form action={submitQuiz.bind(null, courseId, moduleId, typedAssessment.id)} className="flex flex-col gap-6">
