    ...
    <header className="flex items-center justify-between border-b border-neutral-800 bg-[#536270] px-6 py-3">
      ...
      {user && (
        <nav className="flex items-center gap-4 text-sm text-neutral-200">
          {user.role === "admin" && <Link href="/admin/courses" className="hover:text-white">Admin</Link>}
          {user.role === "employer" && <Link href="/employer/roster" className="hover:text-white">Roster</Link>}
          {user.role === "learner" && <Link href="/courses" className="hover:text-white">My Courses</Link>}
